...
import { 
  Car, Calendar, Clock, Wrench, ShieldAlert, CheckCircle, 
  ChevronRight, ChevronLeft, Upload, Info, AlertTriangle 
} from 'lucide-react';
...
export default function Booking() {
  ...
  // Load vehicles and pre-selected service
  useEffect(() => {
    if (currentCustomer) {
    ...
    // Check if redirect has pre-selected service
    if (location.state?.preSelectedService) {
      const svcName = location.state.preSelectedService;
      const foundSvc = availableServices.find(s => s.name === svcName || svcName.includes(s.name));
      if (foundSvc) {
        setSelectedServices([foundSvc]);
      }
    }
  }, [currentCustomer, location.state]);

  const availableServices = [
    { id: 'sb', name: 'Service Berkala', price: 350000, duration: 90, desc: 'Pengecekan 50 titik vital kendaraan' },
    { id: 'tu', name: 'Tune Up Mesin', price: 400000, duration: 60, desc: 'Gurah karbon ruang bakar & injektor' },
    { id: 'go', name: 'Ganti Oli Premium', price: 250000, duration: 30, desc: 'Oli mesin sintetik + filter oli' },
    { id: 'sp', name: 'Spooring 3D', price: 200000, duration: 45, desc: 'Kalibrasi kelurusan roda komputer' },
    { id: 'bl', name: 'Balancing Roda', price: 100000, duration: 30, desc: 'Keseimbangan rotasi ban timbal' },
    { id: 'ac', name: 'Service AC Evaporator', price: 500000, duration: 120, desc: 'Pembersihan AC + sterilisasi kabin' },
    { id: 'km', name: 'Diagnosis Kelistrikan', price: 150000, duration: 60, desc: 'Scan ECU reset error & sensor check' },
  ];
  ...
  const handleNextStep = () => {
    ...
    if (step === 2 && selectedServices.length === 0) {
      toast.error("Silakan pilih minimal 1 jasa servis!");
      return;
    }
    if (step === 3 && (!bookingDate || !bookingTime)) {
      toast.error("Silakan tentukan tanggal dan jam kedatangan!");
      return;
    }
    setStep(prev => prev + 1);
  };
  ...
  const handleSubmitBooking = () => {
    ...
    const newBooking = addBooking(bkgData);
    toast.success("Booking Servis Berhasil Terdaftar!");
    // Redirect to status tracker page
    ...
    <div className="space-y-6 text-left max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm space-y-6 min-h-[420px] flex flex-col justify-between">
          ...
          {step === 1 && (
            <div className="space-y-4 text-left">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {currentCustomer.vehicles ? (
                    currentCustomer.vehicles.map((v, i) => (
                      <button
                        ...
                        className={`p-4 rounded-2xl text-left border transition-all flex items-center justify-between ${
                          selectedVehicle === v 
                            ? 'bg-[#D4E34A]/10 border-[#D4E34A] font-bold text-slate-900' 
                            : 'bg-slate-50 border-slate-100 hover:bg-slate-100'
                        }`}
                      >
                        <div className="text-xs">
                          <div className="font-bold">{v.split(' (')[0]}</div>
                          <div className="text-[10px] text-slate-400 font-semibold mt-1">No Polisi: {v.includes('(') ? v.split('(')[1].replace(')', '') : 'B 1234 ABC'}</div>
                        </div>
                    ...
                    <button
                      type="button"
                      onClick={() => setSelectedVehicle(currentCustomer.vehicle)}
                      className={`p-4 rounded-2xl text-left border transition-all flex items-center justify-between ${
                        selectedVehicle === currentCustomer.vehicle 
                          ? 'bg-[#D4E34A]/10 border-[#D4E34A] font-bold text-slate-900' 
                          : 'bg-slate-50 border-slate-100 hover:bg-slate-100'
                      }`}
                    >
                      <div className="text-xs">
                        <div className="font-bold">{currentCustomer.vehicle.split(' (')[0]}</div>
                        <div className="text-[10px] text-slate-400 font-semibold mt-1">No Polisi: {currentCustomer.vehicle.includes('(') ? currentCustomer.vehicle.split('(')[1].replace(')', '') : 'B 1234 ABC'}</div>
                      </div>
                      <div className="w-5 h-5 rounded-full border border-slate-300 flex items-center justify-center">
                        {selectedVehicle === currentCustomer.vehicle && <div className="w-2.5 h-2.5 rounded-full bg-slate-900" />}
                      </div>
                    </button>
                  )}
                </div>
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex items-start gap-2.5 text-xs text-slate-500 leading-normal">
                ...
                <span>Kendaraan di atas diambil secara otomatis dari histori database CRM pelanggan Anda. Anda juga dapat mendaftarkan plat nomor kendaraan baru.</span>
              </div>
            </div>
          )}
                  ...
                  return (
                    ...
                    >
                      <div className="space-y-1">
                        ...
                        <p className="text-[10px] text-slate-400 font-medium">{svc.desc}</p>
                      </div>
                      <div className="text-right shrink-0 ml-4">
                        <div className="font-black text-slate-950">{formatRupiah(svc.price)}</div>
                        <div className="text-[9px] text-[#A8B330] mt-0.5">{svc.duration} Menit</div>
                      </div>
                    </button>
                  );
                })}
          ...
          {/* STEP 4: COMPLAINTS & PHOTO UPLOAD */}
          {step === 4 && (
            <div className="space-y-4 text-left">
              <h3 className="font-black text-slate-900 text-sm flex items-center gap-2 border-b border-slate-100 pb-3">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-4 items-center">
                    <label className="bg-slate-50 hover:bg-slate-100 border border-dashed border-slate-350 rounded-2xl p-4 cursor-pointer text-center flex flex-col items-center justify-center gap-1.5 min-w-[120px] transition-colors">
          ...
          {step === 5 && (
            <div className="space-y-4 text-left">
              ...
              <div className="bg-slate-50 rounded-2xl p-4 sm:p-5 border border-slate-150 space-y-4 text-xs text-slate-700">
              ...
              <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-3.5 flex items-start gap-2.5 text-xs text-emerald-800">
                ...
                <span>Pemesanan Anda terintegrasi langsung dengan database CRM. Pengingat status pengerjaan akan dikirim secara berkala via WhatsApp.</span>
              </div>
            </div>
          )}
          ...
          <div className="pt-6 border-t border-slate-100 mt-6 flex justify-between gap-4">
            {step > 1 ? (
              <button
                ...
                className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-6 py-3 rounded-xl text-xs transition-all flex items-center gap-1 active:scale-95"
              >
            ...
            {step < 5 ? (
              ...
              <button
                ...
                className="bg-[#D4E34A] text-slate-950 font-bold px-8 py-3.5 rounded-xl text-xs hover:bg-[#C5D33A] transition-all flex items-center gap-1.5 active:scale-95 shadow-md shadow-[#D4E34A]/10"
              >
                <CheckCircle className="w-4.5 h-4.5" />
                <span>Konfirmasi Booking & Hubungkan CRM</span>
              </button>
            )}
          </div>
        ...
        <div className="space-y-4">
          <div className="bg-slate-950 text-white rounded-3xl p-6 border border-slate-800 shadow-sm text-left space-y-5">
            ...
            <div className="space-y-3">
              {selectedServices.map((svc) => (
                <div key={svc.id} className="flex justify-between items-center text-xs border-b border-slate-900 pb-2.5">
                  <div className="text-slate-400 font-semibold">{svc.name}</div>
                  <div className="font-bold text-white">{formatRupiah(svc.price)}</div>
                </div>
              ))}
            ...
            <div className="bg-slate-900/50 rounded-xl p-3 border border-slate-800 text-[10px] text-slate-400 leading-normal flex items-start gap-2">
              ...
              <span>Biaya di atas adalah estimasi jasa pengerjaan dasar. Biaya sparepart tambahan akan diinformasikan mekanik saat inspeksi awal.</span>
            </div>
          </div>
        </div>
